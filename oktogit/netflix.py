...
def ListCountry():
    ...
    CountrySelect = choicebox(
        msg="Select your country",
        title="Netlfix last 7 days additions",
        choices=(countries),
    )
...
def Last7daysNews():
    try:
        ...
        conn.request(
            "GET",
            "/aaapi.cgi?q=get%3Anew7%3A"
            + str(CountryShortCode)
            + str("&p=1&t=ns&st=adv"),
            headers=headers,
        )
        ...
        while count <= key - 1:
            New7Movies = (
                New7Movies
                + (ListNewJson["ITEMS"][count]["title"])
                + str(" - Added the :")
                + str(ListNewJson["ITEMS"][count]["unogsdate"])
                + str(" Type : ")
                + str(ListNewJson["ITEMS"][count]["type"])
                + str("\n")
            )
            count = count + 1
            if count > key - 1:
